...
# Declare Tie if there are no more space in GameBoard
def empty_spaces():
    spaces = 9
    for row in range(3):
        for column in range(3):
...
# Back to menu button
Return_menu_Button = Button(window_play, text= "Return Menu", font=('Arial', 20), bg='#BF0000', borderwidth=-1, padx=7, command= Return_Menu, activeforeground='black', activebackground='#BF0000')
Return_menu_Button.place(anchor=CENTER, relx=.1, rely=.88)

# Reset button
reset_button = Button(window_play, text= "Restart Game", font=('Arial', 20), bg='#CC5500', borderwidth=-1, command= new_game, activeforeground='black', activebackground='#CC5500')
...
for row in range(3):
    for column in range(3):
        buttons[row][column] = Button(GameBoard, text="", font=("arial", 40, "bold"), bg = 'white', activebackground='white', width=5, height=2, command=lambda row=row, column=column: next_turn(row, column), borderwidth=-1)
